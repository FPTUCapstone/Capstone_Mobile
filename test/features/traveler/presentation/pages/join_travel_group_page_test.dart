...
final class _MockTravelGroupRepository
    with _InvitationStub
...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      TravelGroupDetailsRouteArgs? capturedExtra;
      final router = GoRouter(
        ...
        routes: [
          ...
          GoRoute(
            ...
            builder: (_, state) {
              capturedExtra = state.extra as TravelGroupDetailsRouteArgs?;
              return Scaffold(
                body: Text(
      ...
      expect(capturedExtra!.group.id, 42);
      expect(capturedExtra!.group.name, 'Joined Group');
      expect(capturedExtra!.group.itineraryId, 10);
      expect(capturedExtra!.isHost, isFalse);
    },
  );
}
